                      ...
                      <div className="mt-2 flex items-center gap-2">
                        <div className="inline-flex h-9 items-center rounded-lg border border-[var(--line)]">
                          ...
                          <button
                            ...
                            className="focus-ring h-full w-9 rounded-r-lg text-lg font-semibold text-[var(--leaf-deep)] transition active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
                            type="button"
                            disabled={!item.available || item.stock <= 0 || item.quantity >= item.stock}
                            onClick={() => {
                              if (!item.available || item.stock <= 0 || item.quantity >= item.stock) {
                                pushCartNotice(
                                  item.stock <= 0
