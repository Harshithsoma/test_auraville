...
export function AdminReviewsClient() {
  ...
  async function approveReview(reviewId: string) {
    ...
    try {
      await commerceApi.admin.reviews.approve<ApproveReviewResponse>(reviewId);
      await loadReviews();
                    ...
                    <td className="px-3 py-3">
                      <span
                        className={`rounded px-2 py-1 text-[11px] font-semibold ${
                          review.isApproved ? "bg-[var(--mint)] text-[var(--leaf-deep)]" : "bg-[#fff3f2] text-[var(--coral)]"
                        }`}
                      >
            ...
            {reviews.map((review) => (
              <article className="rounded-lg border border-[var(--line)] bg-white p-4" key={review.id}>
                ...
                <p className="text-xs text-[var(--muted)]">
                  {review.productId ?? "general"} | status: {review.isApproved ? "Approved" : "Pending"}
                </p>
