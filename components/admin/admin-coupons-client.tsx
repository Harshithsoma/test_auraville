...
export function AdminCouponsClient() {
  ...
  async function disableCoupon(coupon: AdminCoupon) {
    ...
    try {
      ...
      setListMessage(`Coupon "${coupon.code}" disabled.`);
                      ...
                      <td className="px-3 py-3">
                        <span
                          className={`rounded px-2 py-1 text-[11px] font-semibold ${
                            coupon.isActive ? "bg-[var(--mint)] text-[var(--leaf-deep)]" : "bg-[#fff3f2] text-[var(--coral)]"
                          }`}
                        >
            ...
            <div className="mt-4 grid gap-3 xl:hidden">
              {coupons.map((coupon) => (
                <article className="rounded-lg border border-[var(--line)] bg-white p-4" key={coupon.id}>
                  ...
                  <p className="mt-1 text-xs text-[var(--muted)]">
                    Status: {coupon.isActive ? "Active" : "Disabled"} | Starts: {formatDate(coupon.startsAt)} | Expires:{" "}
