...
import { normalizePhone, PhoneNormalizationError } from "../../utils/phone";
...
async function applyInvalidCredentialsDelay(): Promise<void> {
  const baseDelayMs = 350;
  const jitterMs = randomInt(75, 250);
...
async function createSession(params: {
  ...
  const MAX_REFRESH_TOKEN_CREATE_ATTEMPTS = 3;
  let refreshToken: string | null = null;
  let refreshExpiresAt: Date | null = null;
  let created = false;

  for (let attempt = 0; attempt < MAX_REFRESH_TOKEN_CREATE_ATTEMPTS; attempt += 1) {
    const candidate = signRefreshToken({
      sub: params.user.id,
      familyId
    });
    ...
    try {
      await prisma.refreshToken.create({
        data: {
          ...
          familyId,
          tokenHash: sha256(candidate.token),
          expiresAt: candidate.expiresAt
        }
      });
      refreshToken = candidate.token;
      refreshExpiresAt = candidate.expiresAt;
      created = true;
      break;
    } catch (error) {
  ...
  if (!created || !refreshToken || !refreshExpiresAt) {
    throw new HttpError(
      500,
      "Unable to create authentication session. Please try again.",
      undefined,
...
function safeNormalizePhone(input: string): string {
  try {
    return normalizePhone(input);
  } catch (error) {
    if (error instanceof PhoneNormalizationError) {
      throw new HttpError(400, "Invalid identifier", undefined, "INVALID_IDENTIFIER");
    }
    throw error;
  }
}

function safeResolveIdentifier(input: string): ReturnType<typeof resolveIdentifier> {
  try {
    return resolveIdentifier(input);
  } catch (error) {
    if (error instanceof PhoneNormalizationError) {
      throw new HttpError(400, "Invalid identifier", undefined, "INVALID_IDENTIFIER");
    }
...
export async function sendOtp(email: string): Promise<void> {
  ...
  if (env.NODE_ENV !== "production") {
    ...
    console.log(`OTP generated for ${maskEmail(email)}: REDACTED`);
  }
}
...
export async function signupOtpVerify(params: {
  ...
  const email = params.email?.trim().toLowerCase();
  const phoneNormalized = params.phone ? safeNormalizePhone(params.phone) : null;
...
export async function loginWithPassword(params: {
  ...
  if (!user || !user.isActive || !user.passwordHash) {
    await applyInvalidCredentialsDelay();
...
> {
  ...
  const MAX_REFRESH_TOKEN_CREATE_ATTEMPTS = 3;
  let session: { accessToken: string; refreshToken: string; refreshExpiresAt: Date } | null = null;
