...
export async function enrichCartItems(items: CartPriceItemInput[]): Promise<ValidatedCartItem[]> {
  ...
  const variantByCompositeKey = new Map(
    variants.map((variant) => [`${variant.productId}:${variant.frontendVariantId}`, variant])
  );
  ...
  for (const item of items) {
    ...
    const variant = variantByCompositeKey.get(`${item.productId}:${item.variantId}`);
