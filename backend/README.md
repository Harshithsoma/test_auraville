...
- App: `NODE_ENV`, `PORT`, `FRONTEND_URL`, `CORS_ORIGINS`
- App runtime: `TRUST_PROXY`
- Database: `DATABASE_URL`
- Auth: `JWT_ACCESS_SECRET`, `JWT_REFRESH_SECRET`, `ACCESS_TOKEN_TTL`, `REFRESH_TOKEN_TTL`, `OTP_TTL_MINUTES`, `OTP_MAX_ATTEMPTS`, `OTP_HASH_PEPPER`, `REFRESH_COOKIE_NAME`, `CSRF_COOKIE_NAME`
...
- External API timeout: `EXTERNAL_API_TIMEOUT_MS`
...
- Security:
  - `helmet()` enabled
  - strict CORS allowlist uses `FRONTEND_URL` plus optional `CORS_ORIGINS`
  - refresh token cookie is `httpOnly`, `secure` in production, and `sameSite=none` in production
  - CSRF protection enforced on cookie-based `/auth/refresh` and `/auth/logout` via `X-CSRF-Token` matched to CSRF cookie
...
`GET /health/ready` returns `200` when database connectivity is healthy, otherwise `503`.
